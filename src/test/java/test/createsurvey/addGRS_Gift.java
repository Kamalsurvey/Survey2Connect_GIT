...
public class addGRS_Gift extends configTest{
	WebElement grsIcon;
	Logger testLogger;
	ITestResult result;	
	...
	testLogger=Logger.getLogger(addGRS_Gift.class);

		}


	@Test
	public void clickGRS_Gift() throws InterruptedException
	{
	...
	@Test(dependsOnMethods="clickGRS_Gift")
	public void addGRS_Gift()
	{
		testLogger.info("Creating GRS Gift Question");
		...
		driver.findElement(By.xpath(AddQuestionObjects.grs_QuestionTextArea)).sendKeys("GRS  Gift  Question");
		...
		waitforElementBYXpath(By.xpath(AddQuestionObjects.grs_type_gift));
		WebElement q=driver.findElement(By.xpath(AddQuestionObjects.grs_type_gift));
		q.click();
		
		testLogger.info("Type selected as Gift");
		driver.switchTo().defaultContent();
	...
	for(int i=2;i<=Integer.parseInt(cols);i++){
		...
		testLogger.info("GRS Gift Question has been created successfully");
		}catch(Exception e){
			testLogger.info("Problem occured creating GRS Gift Question .");
			testLogger.error(e.getMessage());
			e.printStackTrace();
			Assert.fail();
		}
	...
	{ 
		...
		{status="PASS";
		generateReport.addPassStatus(testcounter,"ADD GRS Gift ", status);}
		break;
		case ITestResult.FAILURE:
		{status="FAIL";
		generateReport.addFailStatus(testcounter,"ADD  GRS Gift ", status);}
		break;
