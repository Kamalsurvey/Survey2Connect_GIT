...
public class addGRS_Trophy extends configTest{
	WebElement grsIcon;
	Logger testLogger;
	ITestResult result;	
	...
	public addGRS_Trophy()
	{driver=configTest.getDriver();
	testLogger=Logger.getLogger(addGRS_Trophy.class);

		}


	@Test
	public void clickGRS_Trophy() throws InterruptedException
	{
	...
	@Test(dependsOnMethods="clickGRS_Trophy")
	public void addGRS_Trophy()
	{
		testLogger.info("Creating GRS Trophy Question");
		...
		driver.findElement(By.xpath(AddQuestionObjects.grs_QuestionTextArea)).sendKeys("GRS  Trophy  Question");
		...
		waitforElementBYXpath(By.xpath(AddQuestionObjects.grs_type_trophy));
		WebElement q=driver.findElement(By.xpath(AddQuestionObjects.grs_type_trophy));
		q.click();
		
		testLogger.info("Type selected as Trophy");
		driver.switchTo().defaultContent();
	...
	for(int i=2;i<=Integer.parseInt(cols);i++){
		...
		testLogger.info("GRS Trophy Question has been created successfully");
		}catch(Exception e){
			testLogger.info("Problem occured creating GRS Trophy Question .");
			testLogger.error(e.getMessage());
			e.printStackTrace();
			Assert.fail();
		}
	...
	{ 
		...
		{status="PASS";
		generateReport.addPassStatus(testcounter,"ADD GRS Trophy ", status);}
		break;
		case ITestResult.FAILURE:
		{status="FAIL";
		generateReport.addFailStatus(testcounter,"ADD  GRS Trophy ", status);}
		break;
