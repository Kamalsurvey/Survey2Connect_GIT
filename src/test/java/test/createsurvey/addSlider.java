...
public class addSlider extends configTest{
	...
	testLogger=Logger.getLogger(addSlider.class);
	...
	public void addSlider()
	{
		...
		driver.findElement(By.xpath(AddQuestionObjects.dropdown_QuestionTextArea)).sendKeys("Slider Question Continuous Type 1");
		...
		testLogger.info("Slider  Question has been created successfully");
		}catch(Exception e){
			testLogger.info("Problem occured creating Dropdown Slider Question .");
			testLogger.error(e.getMessage());
			e.printStackTrace();
			Assert.fail();
		}
	...
	{ 
		...
		{status="PASS";
		generateReport.addPassStatus(testcounter,"ADD Slider", status);}
		break;
		case ITestResult.FAILURE:
		{status="FAIL";
		generateReport.addFailStatus(testcounter,"ADD Slider", status);}
		break;
