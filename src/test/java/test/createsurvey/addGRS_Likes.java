...
public class addGRS_Likes extends configTest{
	WebElement grsIcon;
	Logger testLogger;
	ITestResult result;	
	...
	testLogger=Logger.getLogger(addGRS_Likes.class);

		}


	@Test
	public void clickGRS_Likes() throws InterruptedException
	{
	...
	@Test(dependsOnMethods="clickGRS_Likes")
	public void addGRS_Likes()
	{
		testLogger.info("Creating GRS Likes Question");
		...
		driver.findElement(By.xpath(AddQuestionObjects.grs_QuestionTextArea)).sendKeys("GRS  Likes  Question");
		...
		waitforElementBYXpath(By.xpath(AddQuestionObjects.grs_type_likes));
		WebElement q=driver.findElement(By.xpath(AddQuestionObjects.grs_type_likes));
		q.click();
		
		testLogger.info("Type selected as Likes");
		driver.switchTo().defaultContent();
	...
	for(int i=2;i<=Integer.parseInt(cols);i++){
		...
		testLogger.info("GRS Likes Question has been created successfully");
		}catch(Exception e){
			testLogger.info("Problem occured creating GRS Likes Question .");
			testLogger.error(e.getMessage());
			e.printStackTrace();
			Assert.fail();
		}
	...
	{ 
		...
		{status="PASS";
		generateReport.addPassStatus(testcounter,"ADD GRS Likes ", status);}
		break;
		case ITestResult.FAILURE:
		{status="FAIL";
		generateReport.addFailStatus(testcounter,"ADD  GRS Likes ", status);}
		break;
