...
public class addGRS_Users extends configTest{
	WebElement grsIcon;
	Logger testLogger;
	ITestResult result;	
	...
	testLogger=Logger.getLogger(addGRS_Users.class);

		}


	@Test
	public void clickGRS_Users() throws InterruptedException
	{
	...
	@Test(dependsOnMethods="clickGRS_Users")
	public void addGRS_Users()
	{
		testLogger.info("Creating GRS Users Question");
		...
		driver.findElement(By.xpath(AddQuestionObjects.grs_QuestionTextArea)).sendKeys("GRS  Users  Question");
		...
		waitforElementBYXpath(By.xpath(AddQuestionObjects.grs_type_users));
		WebElement q=driver.findElement(By.xpath(AddQuestionObjects.grs_type_users));
		q.click();
		
		testLogger.info("Type selected as Users");
		driver.switchTo().defaultContent();
	...
	for(int i=2;i<=Integer.parseInt(cols);i++){
		...
		testLogger.info("GRS Users Question has been created successfully");
		}catch(Exception e){
			testLogger.info("Problem occured creating GRS Users Question .");
			testLogger.error(e.getMessage());
			e.printStackTrace();
			Assert.fail();
		}
	...
	{ 
		...
		{status="PASS";
		generateReport.addPassStatus(testcounter,"ADD GRS Users ", status);}
		break;
		case ITestResult.FAILURE:
		{status="FAIL";
		generateReport.addFailStatus(testcounter,"ADD  GRS Users ", status);}
		break;
