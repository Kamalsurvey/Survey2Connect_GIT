...
public class addGRS_Hearts extends configTest{
	WebElement grsIcon;
	Logger testLogger;
	ITestResult result;	
	...
	public addGRS_Hearts()
	{driver=configTest.getDriver();
	testLogger=Logger.getLogger(addGRS_Hearts.class);

		}


	@Test
	public void clickGRS_Hearts() throws InterruptedException
	{
	...
	@Test(dependsOnMethods="clickGRS_Hearts")
	public void addGRS_TextData()
	{
		testLogger.info("Creating GRS Hearts Question");
		...
		driver.findElement(By.xpath(AddQuestionObjects.grs_QuestionTextArea)).sendKeys("GRS  Hearts  Question");
		...
		waitforElementBYXpath(By.xpath(AddQuestionObjects.grs_type_heart));
		WebElement q=driver.findElement(By.xpath(AddQuestionObjects.grs_type_heart));
		q.click();
		
		testLogger.info("Type selected as Heart");
		driver.switchTo().defaultContent();
	...
	action.sendKeys(Keys.TAB).perform();
		...
		testLogger.info("GRS Heart Question has been created successfully");
		}catch(Exception e){
			testLogger.info("Problem occured creating GRS Heart Question .");
			testLogger.error(e.getMessage());
			e.printStackTrace();
			Assert.fail();
		}
	...
	{ 
		...
		{status="PASS";
		generateReport.addPassStatus(testcounter,"ADD GRS Heart ", status);}
		break;
		case ITestResult.FAILURE:
		{status="FAIL";
		generateReport.addFailStatus(testcounter,"ADD  GRS Heart ", status);}
		break;
