...
public class addGRS_Gradient extends configTest{
	WebElement grsIcon;
	Logger testLogger;
	ITestResult result;	
	...
	public addGRS_Gradient()
	{driver=configTest.getDriver();
	testLogger=Logger.getLogger(addGRS_Gradient.class);

		}


	@Test
	public void clickGRS_Gradient() throws InterruptedException
	{
	...
	@Test(dependsOnMethods="clickGRS_Gradient")
	public void addGRS_Dislike()
	{
		testLogger.info("Creating GRS Gradient Question");
		...
		driver.findElement(By.xpath(AddQuestionObjects.grs_QuestionTextArea)).sendKeys("GRS  Gradient  Question");
		...
		waitforElementBYXpath(By.xpath(AddQuestionObjects.grs_type_smily_Grad));
		WebElement q=driver.findElement(By.xpath(AddQuestionObjects.grs_type_smily_Grad));
		q.click();
		
		testLogger.info("Type selected as Gradient");
		driver.switchTo().defaultContent();
	...
	for(int i=2;i<=Integer.parseInt(cols);i++){
		...
		testLogger.info("GRS Gradient Question has been created successfully");
		}catch(Exception e){
			testLogger.info("Problem occured creating GRS Gradient Question .");
			testLogger.error(e.getMessage());
			e.printStackTrace();
			Assert.fail();
		}
	...
	{ 
		...
		{status="PASS";
		generateReport.addPassStatus(testcounter,"ADD GRS Gradient ", status);}
		break;
		case ITestResult.FAILURE:
		{status="FAIL";
		generateReport.addFailStatus(testcounter,"ADD  GRS Gradient ", status);}
		break;
