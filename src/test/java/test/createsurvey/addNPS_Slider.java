...
public class addNPS_Slider extends configTest{
	WebElement npsIcon;
	Logger testLogger;
	ITestResult result;	
	

	public addNPS_Slider()
	{driver=configTest.getDriver();
	testLogger=Logger.getLogger(addNPS_Slider.class);
	...
	public void addNPS()
	{
		testLogger.info("Creating NPS Slider Question");
		...
		driver.findElement(By.xpath(AddQuestionObjects.NPS_QuestionTextArea)).sendKeys("NPS Slider Question");
		driver.switchTo().defaultContent();
		WebElement e3=configTest.getDriver().findElement(By.xpath(AddQuestionObjects.NPS_Type));
		Select select=new Select(e3);
		select.selectByVisibleText("Slider");
		...
		testLogger.info("NPS Slider Question has been created successfully");
		}catch(Exception e){
	...
	{ 
		...
		{status="PASS";
		generateReport.addPassStatus(testcounter,"ADD NPS_Slider", status);}
		break;
		case ITestResult.FAILURE:
		{status="FAIL";
		generateReport.addFailStatus(testcounter,"ADD NPS_Slider", status);}
		break;
