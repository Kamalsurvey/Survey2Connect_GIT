...
public class addNPS_Arc extends configTest{
	WebElement npsIcon;
	Logger testLogger;
	ITestResult result;	
	...
	testLogger=Logger.getLogger(addNPS_Arc.class);
	...
	public void addNPS()
	{
		testLogger.info("Creating NPS Arc Question");
		...
		driver.findElement(By.xpath(AddQuestionObjects.NPS_QuestionTextArea)).sendKeys("NPS Arc Question");
		driver.switchTo().defaultContent();
		WebElement e3=configTest.getDriver().findElement(By.xpath(AddQuestionObjects.NPS_Type));
		Select select=new Select(e3);
		select.selectByVisibleText("Nps Arc");
		...
		while(c.hasNext())
		{
			...
			if(x.getText().equals(("Color") )){x.click();
		...
		testLogger.info("NPS Arc Question has been created successfully");
		}catch(Exception e){
	...
	{ 
		...
		{status="PASS";
		generateReport.addPassStatus(testcounter,"ADD NPS_Arc", status);}
		break;
		case ITestResult.FAILURE:
		{status="FAIL";
		generateReport.addFailStatus(testcounter,"ADD NPS_Arc", status);}
		break;
