...
public class addGRS_Dislike extends configTest{
	WebElement grsIcon;
	Logger testLogger;
	ITestResult result;	
	...
	{
		if(!excelConfig.runnabletest.contains("addGRS_Dislike")){throw new SkipException("");}		
	}

	public addGRS_Dislike()
	{driver=configTest.getDriver();
	testLogger=Logger.getLogger(addGRS_Dislike.class);

		}


	@Test
	public void clickGRS_Dislike() throws InterruptedException
	{
	...
	@Test(dependsOnMethods="clickGRS_Dislike")
	public void addGRS_Dislike()
	{
		testLogger.info("Creating GRS Dislike( Question");
		...
		driver.findElement(By.xpath(AddQuestionObjects.grs_QuestionTextArea)).sendKeys("GRS  Dislike  Question");
		...
		waitforElementBYXpath(By.xpath(AddQuestionObjects.grs_type_dislikes));
		WebElement q=driver.findElement(By.xpath(AddQuestionObjects.grs_type_dislikes));
		q.click();
		
		testLogger.info("Type selected as Dislike(");
		driver.switchTo().defaultContent();
	...
	for(int i=2;i<=Integer.parseInt(cols);i++){
		...
		testLogger.info("GRS dislikes Question has been created successfully");
		}catch(Exception e){
			testLogger.info("Problem occured creating GRS dislikes Question .");
			testLogger.error(e.getMessage());
			e.printStackTrace();
			Assert.fail();
		}
	...
	{ 
		...
		{status="PASS";
		generateReport.addPassStatus(testcounter,"ADD GRS Dislike ", status);}
		break;
		case ITestResult.FAILURE:
		{status="FAIL";
		generateReport.addFailStatus(testcounter,"ADD  GRS Dislike ", status);}
		break;
