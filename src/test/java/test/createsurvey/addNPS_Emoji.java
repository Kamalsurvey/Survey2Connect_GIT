...
public class addNPS_Emoji extends configTest{
	WebElement npsIcon;
	Logger testLogger;
	ITestResult result;	
	...
	testLogger=Logger.getLogger(addNPS_Emoji.class);
	...
	public void addNPS()
	{
		testLogger.info("Creating NPS Emojis Question");
		...
		driver.findElement(By.xpath(AddQuestionObjects.NPS_QuestionTextArea)).sendKeys("NPS  Emojis Question");
		driver.switchTo().defaultContent();
		WebElement e3=configTest.getDriver().findElement(By.xpath(AddQuestionObjects.NPS_Type));
		Select select=new Select(e3);
		select.selectByVisibleText("Emojis");
		...
		testLogger.info("NPS Emojis Question has been created successfully");
		}catch(Exception e){
	...
	{ 
		...
		{status="PASS";
		generateReport.addPassStatus(testcounter,"ADD NPS_Emojis", status);}
		break;
		case ITestResult.FAILURE:
		{status="FAIL";
		generateReport.addFailStatus(testcounter,"ADD NPS_Emojis", status);}
		break;
