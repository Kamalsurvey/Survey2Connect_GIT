...
public class addGRS_Stars extends configTest{
	WebElement grsIcon;
	Logger testLogger;
	ITestResult result;	
	...
	testLogger=Logger.getLogger(addGRS_Stars.class);

		}


	@Test
	public void clickGRS_Starts() throws InterruptedException
	{
	...
	@Test(dependsOnMethods="clickGRS_Starts")
	public void addGRS_TextData()
	{
		...
		driver.findElement(By.xpath(AddQuestionObjects.grs_QuestionTextArea)).sendKeys("GRS  Stars  Question");
		...
		waitforElementBYXpath(By.xpath(AddQuestionObjects.grs_type_stars));
		WebElement q=driver.findElement(By.xpath(AddQuestionObjects.grs_type_stars));
		q.click();
		
		testLogger.info("Type selected as Stars");
		driver.switchTo().defaultContent();
	...
	action.sendKeys(Keys.TAB).perform();
		...
		testLogger.info("GRS Stars Question has been created successfully");
		}catch(Exception e){
			testLogger.info("Problem occured creating GRS Stars Question .");
			testLogger.error(e.getMessage());
			e.printStackTrace();
			Assert.fail();
		}
	...
	{ 
		...
		{status="PASS";
		generateReport.addPassStatus(testcounter,"ADD GRS Stars ", status);}
		break;
		case ITestResult.FAILURE:
		{status="FAIL";
		generateReport.addFailStatus(testcounter,"ADD  GRS Stars ", status);}
		break;
