...
public class addGRS_Smileys extends configTest{
	WebElement grsIcon;
	Logger testLogger;
	ITestResult result;	
	...
	public addGRS_Smileys()
	{driver=configTest.getDriver();
	testLogger=Logger.getLogger(addGRS_Smileys.class);

		}


	@Test
	public void clickGRS_Smileys() throws InterruptedException
	{
	...
	@Test(dependsOnMethods="clickGRS_Smileys")
	public void addGRS_Dislike()
	{
		testLogger.info("Creating GRS Smileys( Question");
		...
		driver.findElement(By.xpath(AddQuestionObjects.grs_QuestionTextArea)).sendKeys("GRS  Smileys  Question");
		...
		waitforElementBYXpath(By.xpath(AddQuestionObjects.grs_type_smily));
		WebElement q=driver.findElement(By.xpath(AddQuestionObjects.grs_type_smily));
		q.click();
		
		testLogger.info("Type selected as Smileys");
		driver.switchTo().defaultContent();
	...
	for(int i=2;i<=Integer.parseInt(cols);i++){
		...
		testLogger.info("GRS Smileys Question has been created successfully");
		}catch(Exception e){
			testLogger.info("Problem occured creating GRS Smileys Question .");
			testLogger.error(e.getMessage());
			e.printStackTrace();
			Assert.fail();
		}
	...
	{ 
		...
		{status="PASS";
		generateReport.addPassStatus(testcounter,"ADD GRS Smileys ", status);}
		break;
		case ITestResult.FAILURE:
		{status="FAIL";
		generateReport.addFailStatus(testcounter,"ADD  GRS Smileys ", status);}
		break;
